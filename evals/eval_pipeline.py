...
def hit_api_with_questions() -> List[str]:
    ...
    if not os.path.exists(DATASET_PATH):
        print(f" Warning: Dataset file not found at {DATASET_PATH}. Skipping API seed.")
        return []
...
def fetch_latest_traces(limit: int) -> List[Dict[str, Any]]:
    ...
    for idx, r in enumerate(langgraph_runs, 1):
        ...
        if retrieved_chunks:
            ...
            print("    No Chunks Captured (Query might have hit Guardrail or Direct Agent Response)")
